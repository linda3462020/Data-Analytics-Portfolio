...
```{r}
data1 <- filter(orig_svcg, first_record == TRUE)
gg <- ggplot(data1, aes(x=fico)) +
  geom_histogram(binwidth = 5, fill = "firebrick", color = "white") +
  theme_light()
gg

## Ans: The higher fico score of the applicant, the more opportunity to have access to mortgages.
```
...
```{r}
data1 <- filter(orig_svcg, first_record == TRUE & svcg_cycle >= 200501 & svcg_cycle <= 200712)
d_summary <- data1 %>%
  group_by(st) %>%
  summarize(
    num_default = sum(ever_default == 1),
    total_loans = n(),
    default_rate = num_default / total_loans)
gg <- ggplot(d_summary, aes(x=st, y=default_rate, color=as.factor(st))) +
...
```{r}
data1 <- mutate(orig_svcg, date = as.Date(paste0(svcg_cycle, "01"), format="%Y%m%d"))
rate1 <-  data1 %>%
  group_by(date) %>%
  summarize(d_rate1 = mean(ever_default))
gg <- ggplot(rate1, aes(x=date, y=d_rate1)) +
  geom_line(color = "firebrick") +
  scale_x_date(date_labels = "%b %Y", date_breaks = "1 year") + 
  theme_light()
gg

### Ans: Yes, the default rate shows more loan defaults from 2007 to 2016.
```
...
d_summary <- data1 %>%
  ...
  summarize(
    ...
    total_loans = n(),
    default_rate = num_default / total_loans) %>%
  filter(!is.na(default_rate), default_rate >= 0 & default_rate <= 1)

gg <- ggplot(d_summary, aes(x = fico, y = default_rate)) +
  geom_point(color="firebrick", alpha=1) + 
  scale_y_continuous(limits = c(0, 1)) +
  theme_light() +
  labs(x = "FICO",
    y = "Default Rate",
    title = "Default rates by FICO score") +
